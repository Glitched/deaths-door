...
**Game State** (`game_state.rs`): Immutable `GameState`/`PlayerState` structs (owned, cloned on update). Derived methods include `living_player_count()`, `execution_threshold()`, and `get_dead_players_with_vote()`. Also tracks an optional `chopping_block` (player up for execution + optional vote count) that auto-clears in `apply()` when that player dies or is removed, or when night begins. Updated via `GameManager::dispatch()` which atomically applies, persists, and notifies SSE subscribers.

**Day/night cycle** (`apply.rs` + `routes/game.rs`): `GameState.phase` (day/night) drives the app's day/night modes; games open on the first night. `POST /game/day/begin` starts a day (increments `day_number`); night deaths queue in `deaths_to_announce` and are announced one at a time via `POST /game/day/announce_death` (fires the death scene). `POST /game/nominations` records a confirmed vote `{player_name, voters, votes?}`: voters must be in `eligible_voters` (living, or dead holding a dead vote — dead votes are spent automatically), each player can be nominated once per day (`nominations_today`), and the chopping block resolves in `apply()` — meeting `execution_threshold` and beating the standing count (current block votes, or `tied_votes` where earlier nominations tied) takes the block; exactly matching the standing count empties the block and the tie stands for the rest of the day (`votes_to_take_block` surfaces the live number to beat); fewer votes change nothing. `POST /game/day/end` executes whoever is on the block (`PlayerExecuted` event, public — not queued for announcement); `POST /game/night/begin` enters night (clears nominations/block; a night after any day is not the first night). The phase also stays coherent when the night-step bookmark is set directly ("Dawn" = day). The auto death effect only fires for daytime deaths — night kills stay silent. Day-cycle/nomination routes live in `routes/day.rs`.

**Live vote tally** (`vote.rs`): `POST /game/vote/start` opens an ephemeral tally (not event-sourced, like the timer) surfaced as `vote_in_progress` in `/game/state`/SSE so the overlay shows the count climbing; `POST /game/vote/voters` replaces the voter selection (full list each time), `POST /game/vote/cancel` abandons it, and `POST /game/vote/confirm` records the nomination through the same path as `POST /game/nominations`. The tally clears on phase changes, `day/end`, game switches (new/load/rewind/fork), and game end; a failed confirm restores it.

**Game over & journal**: the state carries a derived `game_over_hint` (demon dead, or two players left with the demon standing — a hint, never a ruling) and `winner` once `POST /game/end {winner}` records the outcome (400 on re-end; rewind to change). `GET /game/journal` renders the event log as Markdown — sections per day/night, one line per event, winner at the bottom — ready to paste into the group chat.
...
**Served pages** (`app.rs`): besides the JSON API, the backend serves the built React app from `static/app` (fallback route — `/overlay` is the projector overlay), a standalone role-reveal screen at `/reveal` (`static/role.html`), and an interactive dev console at `/debug` (`static/debug.html`). The `/debug` console subscribes to the SSE stream and exposes buttons that drive game state (day cycle, live vote tally, night phase, reveals, etc.) — handy for testing without the iOS app. `/static/*` serves the remaining assets.
